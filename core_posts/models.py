...
class BedRoom(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.availability_from and self.availability_till:
            ...
            self.available_days = 0  # Default to 0 if dates are not provided
...
class Tables(models.Model):
    ...
    restaurant = models.ForeignKey(Restaurant, on_delete=models.CASCADE, db_index=True, default=None) #Default for dummy
    review = models.ManyToManyField('Review', db_index=True, blank=True) #Default for dummy
    capacity = models.PositiveIntegerField(default=0)
    price = models.PositiveIntegerField(default=0)
    availability_from = models.DateField(null=True, default=None)
    availability_till = models.DateField(null=True, default=None)
...
class FavouriteList(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, db_index=True,  default='')
