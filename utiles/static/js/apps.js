            ...
            try {
                ...
                if (response.ok) {
                    ...
                    result.hotel_data.forEach(ex => {
                        ...
                        image.alt = ex.hotel.name || 'Unknown Hotel'; // Default alt text for better UX
                        image.src = ex.image || '/static/images/default-hotel.jpg'; // Default image if none provided
                        ...
                        favoriteButton.addEventListener('click', (e) => {
                            ...
                            // Toggle between dark pink and default color for heart icon
                            if (currentColor === 'currentColor') {
                                ...
                                // Encode room_id and save in localStorage
                                const encodedRoomId = btoa(ex.room_id); // Encode the room_id
                                let unAuthValues = JSON.parse(localStorage.getItem('un_auth_values')) || [];

                                // Check if the encodedRoomId already exists
                                if (!unAuthValues.includes(encodedRoomId)) {
                                    unAuthValues.push(encodedRoomId);
                                    localStorage.setItem('un_auth_values', JSON.stringify(unAuthValues));
                                }
                            } else {
                                ...
                                // Remove the encoded room_id from localStorage
                                const encodedRoomId = btoa(ex.room_id); // Encode the room_id
                                let unAuthValues = JSON.parse(localStorage.getItem('un_auth_values')) || [];

                                // Remove the encodedRoomId if it exists
                                unAuthValues = unAuthValues.filter(id => id !== encodedRoomId);
                                localStorage.setItem('un_auth_values', JSON.stringify(unAuthValues));
                            }

                            // Add your favorite functionality logic here (e.g., API call)
                        });
                        ...
                        cardTitle.textContent = ex.hotel.name || 'Unknown Hotel'; // Default text for better UX

                        // Attach event listener to the cardTitle for redirection with security
                        ...
                        locationText.textContent += ` ${ex.hotel.city.name || 'Unknown City'}, ${ex.hotel.country || 'Unknown Country'}`;
                        ...
                        bedText.textContent += ` ${ex.room_type || 'Unknown Room Type'}`;
...
function searchHotelRooms(section = '') {
    ...
    const searchUrl = section === 'a' ? '/nakiese/search-h' : '/nakiese/search-res';
